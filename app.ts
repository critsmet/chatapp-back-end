  ...
  .then(async ({ iceServers }) => {
    ...
    io.on("connection", (socket) => {
      ...
      socket.on("initializeSession", (username) => {
        ...
        console.log(
          `User ${user.username} (${socket.id}) has joined. Total users: ${users.length}`
        );
      });
      ...
      socket.on("disconnect", () => {
        ...
        if (disconnectedUser) {
          ...
          activeBroadcasters = activeBroadcasters.filter(
            (id) => id !== socket.id
          );
          socket.broadcast.emit("userLogout", disconnectedUser);
          console.log(
            `User ${disconnectedUser.username} (${socket.id}) has disconnected. Total users: ${users.length}`
          );
        } else {
      ...
      socket.on("sendMessage", (text) => {
        ...
        if (user) {
          ...
          console.log(
            `Couldn't find user with socketId ${socket.id} for message`
          );
        }
      });
      ...
      socket.on("requestBroadcast", () => {
        ...
        if (activeBroadcasters.length >= 4) {
          ...
          console.log(
            `Broadcast denied - max limit reached (${activeBroadcasters.length}/4)`
          );
          return;
        }
        ...
        if (activeBroadcasters.includes(socket.id)) {
          socket.emit("broadcastRequestResponse", { approved: false });
          console.log(
            `Broadcast denied - user ${socket.id} already broadcasting`
          );
          return;
        }
        ...
        console.log(
          `Broadcast approved for ${socket.id}. Active broadcasters: ${activeBroadcasters.length}/4`
        );
      });
      ...
      socket.on("endBroadcast", () => {
        ...
        console.log(
          `Broadcast ended by ${socket.id}. Active broadcasters: ${activeBroadcasters.length}/4`
        );
      });
      ...
      socket.on("connectionFailed", (targetSocketId) => {
        console.log(
          `Connection failed between ${socket.id} and ${targetSocketId}`
        );
      });
    });
